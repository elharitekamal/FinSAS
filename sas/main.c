...
printf("\n * *  * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *  \n");
    int indice = 0 ;
    char sup[50];
    printf("\nEntrez le code du medicament pour le supprimer : ");
    scanf("%s", sup);
    for(i=0; i<N; i++){
        if(strcmp(sup, M[i].code) == 0){
            ...
            N--;
            printf("L'operation s'est effectuee avec succes.\n ");
                  }}
...
void acheter(){
    int indice = 0;
    char ach[50];
    printf("\nEntrez le code de ton medicament pour l'acheter : ");
    scanf("%s", ach);
    for(i=0; i<N; i++){
        if(strcmp(ach, M[i].code) == 0){
        indice = 1;
        printf("Tapez combien de medicament vous voulez :\n");
        scanf("%d", &nbr);
        if(nbr <= M[i].quantite){

                M[i].quantite = M[i].quantite - nbr;

                M[i].total = (M[i].prix*nbr);
                S = S + M[i].total;



            tprd = tprd + nbr;
            printf("L'opperation s'est effectuee avec succes .\n");
        }
        else
            printf("Cette quantitee est invallide pour le moment. \n");
        }
...
void maxtri() {
          ...
          w=M[j].total;
          M[j].total=M[j+1].total;
          M[j+1].total=w;
            }
            }
            }
      printf("Le max des des prix des produits vendus est %s de totale de %.2f DH .\n", M[0].nom, M[0].total);
...
void mintri(){
        ...
        printf("Le min des des prix des produits vendus est %s de totale de %.2f DH .\n", M[nbr-1].nom, M[nbr-1].total);
}
...
void statistique(){
int stat;
printf("Tapez [1] - Pour le total des prix des produits.\n");
printf("Tapez [2] - Pour la moyenne des prix des produits.\n");
printf("Tapez [3] - Pour le max des prix des produits vendus.\n");
printf("Tapez [4] - Pour le min des prix des produits vendus.\n");
scanf("%d", &stat);
switch(stat){
case 1 :
        printf("Le Total des prix des produits vendus est %.2f DH Le %d / %d / %d \n", S, jr, ms, an);
        break;

case 2 :
        printf("La moyenne des prix des produits vendus est %.2f DH Le %d / %d / %d \n", S/tprd, jr, ms, an);
        break;
case 3 :
        maxtri(); break;
...
case 4 :
        mintri(); break;
default:
     printf("Pardon cette option n'exixte pas. \n");
...
{
int tri, rech, op;
printf("\n                                   ----   BIENVENU CHER UTILISATAEUR:   ----\n");
printf("\n   Avant de demarrer le programme veuillez initialiser la date d'aujourd'hui s'il vous plait :\n");
printf("Jour : ");
scanf("%d", &jr);
printf("Mois :");
scanf("%d", &ms);
